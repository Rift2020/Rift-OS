...
impl<const ORDER: usize> LockedHeap<ORDER> {
    ...
    pub fn borrow_alloc(&self,layout: Layout)-> *mut u8{
        let count = (max(
            layout.size().next_power_of_two(),
            max(layout.align(), size_of::<usize>()),
        ...
        self.0.lock().alloc(layout).ok().map_or(0 as *mut u8, |allocation| allocation.as_ptr())
    }
...
unsafe impl<const ORDER: usize> GlobalAlloc for LockedHeap<ORDER> {
    unsafe fn alloc(&self, layout: Layout) -> *mut u8 {
        ...
        if let Ok(ptr)=ptr {
            return Some(ptr).map_or(0 as *mut u8, |allocation| allocation.as_ptr());
        }
        else{
            let see=self.borrow_alloc(layout);
            return see;
        }         
}
...
/// ```
/// use buddy_system_allocator:.map_or(0 as *mut u8, |allocation| allocation.as_ptr()):*;
